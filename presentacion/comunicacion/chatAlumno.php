...
<div class="container-fluid row justify-content-center">
	<div class="row mt-3">
        ...
        echo "<script>setTimeout(\"location.href = 'index.php?pid=" . base64_encode("presentacion/comunicacion/chatAlumno.php") . "&nombre=" . $_GET["nombre"] . "';\",1500);</script>";
        ?>
    						</div>
    					<?php } ?>
    ...
    $docente->consultar();
    $totalR = $chat->registros($docente->getNombre(), $_GET["nombre"]);
    $mensajes = $chat->consultar($docente->getNombre(), $_GET["nombre"]);
...
foreach ($mensajes as $mensajeActual) {
            ...
            if ($fecha == 1) {
                ?>
								...
								<b> <?php echo $mensajeActual->getFecha();?> </b>
                                   	  	...
                                   	  	<?php }elseif ($i==0){?>     
								<div style="font-size: 18px; background-color: #ABB2B9";>
								<b> <?php echo $mensajeActual->getFecha();?> </b>
							</div>
							<br>
                                   		<?php } if ($mensajeActual->getNombre()==$docente->getNombre()){?>                                     			                                   		    
                                           			<div
    ...
    ?>
                    	<form class="form-horizontal"
							action="<?php echo "index.php?pid=" . base64_encode("presentacion/comunicacion/chatAlumno.php") . "&nombre=" . $_GET["nombre"]?> "
							method="post">
							<div class="form-group" style="margin: 5px;">
								<input type="hidden" name="id" class="form-control" placeholder="Nombre" required="required" value="<?php echo $id?>">
							</div>
							<div class="form-group" style="margin: 5px;">
								<input type="hidden" name="nombre" class="form-control" placeholder="Nombre" required="required" value="<?php echo $docente->getNombre()?>">
							</div>
							<div class="form-group" style="margin: 5px;">
