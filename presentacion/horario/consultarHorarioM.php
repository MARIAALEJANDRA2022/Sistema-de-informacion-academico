...
if ($_SESSION["rol"] == "administrador") {
    ...
    $horarios = $horario->consultarPorPagina($cantidad, $pagina, $orden, $dir);
...
<div class="container-fluid row justify-content-center">
    ...
    foreach ($horarios as $horarioActual) {
        ...
        $materia = new curso($horarioActual->getCodigo_FK(), $horarioActual->getGrupo());
        $materia ->consultar();
    ...
    if ($pagina == 1) {
        ...
        echo "<li class='page-item'><a class='page-link' href='index.php?pid=" . base64_encode("presentacion/horario/consultarHorarioM.php") . "&pagina=" . ($pagina - 1) . "&cantidad=" . $cantidad . (($orden != "") ? "&orden=" . $orden : "") . (($dir != "") ? "&dir=" . $dir : "") . "'>Anterior</a></li>";
    }
            ...
            if ($pagina == $i) {
                ...
                if ($pagina != 1 && $i == $pagina - 2 || $pagina != 1 && $i == $pagina + 2) {
                    if ($i == 1 || $i == $totalPaginas) {
                        echo "<li class='page-item'><a class='page-link' href='index.php?pid=" . base64_encode("presentacion/horario/consultarHorarioM.php") . "&pagina=" . $i . "&cantidad=" . $cantidad . (($orden != "") ? "&orden=" . $orden : "") . (($dir != "") ? "&dir=" . $dir : "") . "'>" . $i . "</a></li>";
                    } else {
                        echo "...";
                    }
                } else {
    ...
    if ($pagina == $totalPaginas || $totalRegistros == 0) {
        ...
        echo "<li class='page-item'><a class='page-link' href='index.php?pid=" . base64_encode("presentacion/horario/consultarHorarioM.php") . "&pagina=" . ($pagina + 1) . "&cantidad=" . $cantidad . (($orden != "") ? "&orden=" . $orden : "") . (($dir != "") ? "&dir=" . $dir : "") . "'>Siguiente</a></li>";
    }
    ?>
    								</ul>
							</nav>
...
<script>
    $("#cantidad").on("change", function() {
    	url = "index.php?pid=<?php echo base64_encode("presentacion/horario/consultarHorarioM.php") ?>&cantidad=" + $(this).val() + "<?php echo (($orden!="")?"&orden=" . $orden:"") . (($dir!="")?"&dir=" . $dir:"") ?>";
    	//alert (url);
